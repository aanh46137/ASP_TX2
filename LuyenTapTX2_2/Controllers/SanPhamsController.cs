...
using LuyenTapTX2_2.Models;

namespace LuyenTapTX2_2.Controllers
{
    public class SanPhamsController : Controller
    {
        ...
        {
            int tongSoSP = db.SanPhams.Count(x => x.DonGia >= 12000);
        ...
        public ActionResult Index( string searchString )
        {
            ...
            if ( !String.IsNullOrEmpty(searchString))
            {
                String x = Request.Form["searchtype"];
                 sanPhams = db.SanPhams.Where(p => p.TenSP.Contains(searchString));
                //int searchInt = int.Parse(searchString);
                //sanPhams = db.SanPhams.Where(p => p.DonGia >= searchInt);
            }
            ViewBag.ds2 = db.SanPhams.OrderByDescending(p => p.MaSP).Take(10).ToList();
            //ViewBag.ds3 = db.SanPhams.OrderByDescending(p => p.DonGia).Take(10).ToList();
