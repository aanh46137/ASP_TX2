...
namespace WebApp3.Controllers
{
    public class ProductsController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            //             .Select(g => g.First().ProductID).ToList();   // fetch the corresponding product

            //var top8valuest = db.Products
            //             .GroupBy(view => view.Price)   // group views by product
            //             .OrderByDescending(g => g.Count()) // order from most- to least-viewed
            //             .Take(8)                           // take the top 8
            //             .Select(g => g.First().ProductID).ToList();   // fetch the corresponding product

            //ViewBag.top8first = top8first;
            //return View(top8valuest);
